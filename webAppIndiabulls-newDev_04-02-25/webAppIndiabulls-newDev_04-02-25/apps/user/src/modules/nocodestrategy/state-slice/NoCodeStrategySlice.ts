...
import {
  LegItem,
  LegItemPayload,
  SetDropdownAPIFailPayload,
  SetDropdownDataLoadingPayload,
  ShowEditDIYPayload,
...
export interface PageConfig {
  page: InitialStateInterface["currentPage"];
  showMySaved: boolean;
}
...
export const InitialState: InitialStateInterface = {
  ...
  rePopulateForm: 0,
  DIYForm: false,
  dropdownAPIFail: false,
  dropdownDataLoadingState: true,
};
...
const NoCodeStrategySlice = createSlice({
  ...
  reducers: {
    ...
    saveLegData(state, action: LegItemPayload) {
      state.legData = Array.isArray(action.payload)
        ? action.payload
        : [action.payload];
    },
    clearLegData(state) {
    ...
    showEditDIYForm(state, action: ShowEditDIYPayload) {
      state.DIYForm = action.payload.DIYForm;
    },
    setDropDownAPIFailState(state, action: SetDropdownAPIFailPayload) {
      state.dropdownAPIFail = action.payload.dropdownAPIFail;
    },
    setDropdownLoadingState(state, action: SetDropdownDataLoadingPayload) {
      state.dropdownDataLoadingState = action.payload.dropdownDataLoadingState;
    }
  },
});
