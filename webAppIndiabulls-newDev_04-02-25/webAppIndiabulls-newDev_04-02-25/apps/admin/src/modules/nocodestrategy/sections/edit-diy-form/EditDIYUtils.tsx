...
export const NoCodeStrategySchema = (isLive: boolean | null) =>
  z
    ...
    // .refine(
    //   (data) => {
    //     const [entryHour, entryMinute] = data.entryTime.split(":").map(Number);
    //     const [exitHour, exitMinute] = data.exitTime.split(":").map(Number);

    //     const entryTotalMinutes = entryHour * 60 + entryMinute;
    //     const exitTotalMinutes = exitHour * 60 + exitMinute;

    //     // Ensure exitTime is not earlier than entryTime
    //     return exitTotalMinutes >= entryTotalMinutes;
    //   },
    //   {
    //     message: "cannot be earlier than entry time",
    //     path: ["exitTime"], // Focuses error on the `exitTime` field
    //   }
    // )
    // // .refine(
    // //   (data) =>
    // //     data.profitMTMDropDown !== "PercentOfCapital" ||
    // //     (data.profitMTMValue && data.profitMTMValue <= 100),
    // //   {
    ...
    // .refine(
    //   (data) =>
    //     data.stopLossMTMDropDown !== "PercentOfCapital" ||
    //     (data.stopLossMTMValue && data.stopLossMTMValue <= 100),
    //   {
    //     message: "value cannot exceed 100",
